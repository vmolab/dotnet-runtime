...
// Generate code for power-of-2 multiplication.
void CodeGen::genCodeForMul(GenTreeOp* treeNode)
{
    // fprintf(stderr, "[CLAMP] operation %d\n", treeNode->OperGet());
    assert(treeNode->OperIs(GT_MUL)); // GT_ADD: 53, GT_AND: 62
    ;
...
void CodeGen::genCodeForTreeNode(GenTree* treeNode)
{
    ...
    switch (treeNode->gtOper)
    {
        ...
        case GT_SUB:
            ...
            break;
        case GT_MUL:
            fprintf(stderr, "[CLAMP] GT_MUL %d\n", GT_MUL);
            fprintf(stderr, "[CLAMP] gtOper %d\n", treeNode->gtOper);
            fprintf(stderr, "[CLAMP] Operget %d\n", treeNode->AsOp()->OperGet());
            genConsumeOperands(treeNode->AsOp());
